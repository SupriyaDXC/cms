...
public class WalletDAO {
	...
	public boolean updateWalletOnCancle(int custId,WalType walType,int ordAmt) throws SQLException {
		Connection con = ConnectionHelper.getConnection();
		String cmd = "update wallet set wal_amount=wal_amount+? where Cus_Id=? AND WAL_TYPE=?";
		PreparedStatement pst = con.prepareStatement(cmd);
		pst.setInt(1, ordAmt);
		pst.setInt(2, custId);
		String walletType=String.valueOf(walType);
		pst.setString(3, walletType);
		pst.executeUpdate();
		return true;
	}
